import XCTest
@testable import Growlightlog

final class GrowlightlogTests: XCTestCase {
    ...
    func test_addEntry_increasesCount() {
        ...
        let entry = Entry(setupName: "Test", date: Date(), note: "note")
    ...
    func test_addEntry_blockedAtFreeLimit() {
        ...
        while store.entries.count < Store.freeLimit {
            _ = store.add(Entry(setupName: "Filler", date: Date(), note: ""), isPro: false)
        }
        let result = store.add(Entry(setupName: "Overflow", date: Date(), note: ""), isPro: false)
    ...
    func test_addEntry_proBypassesLimit() {
        ...
        let result = store.add(Entry(setupName: "ProEntry", date: Date(), note: ""), isPro: true)
    ...
    func test_deleteEntry_removesFromList() {
        let store = Store()
        let entry = Entry(setupName: "ToDelete", date: Date(), note: "")
        _ = store.add(entry, isPro: false)
        store.delete(entry)
    ...
    func test_updateEntry_changesFields() {
        let store = Store()
        var entry = Entry(setupName: "Original", date: Date(), note: "")
        _ = store.add(entry, isPro: false)
        entry.setupName = "Updated"
        store.update(entry)
        XCTAssertEqual(store.entries.first(where: { $0.id == entry.id })?.setupName, "Updated")
    }
